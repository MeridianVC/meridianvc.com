...
const Testimonial: FC<TestimonialProps> = ({ alignment, statement, name, company, imageSrc, left }) => {
  ...
  words.pop();
  const trimmedStatement = words.join(' ');
  const lastWord = statement.trim().split(/\s+/).pop();
    ...
    <div style={containerStyle}>
      ...
      <div className="testimonialContent">
        <Text variant="BodyBaskerville" className="testimonialStatement" style={statementStyle}>
          ...
          {trimmedStatement + ' '}
          <span style={lastWordStyle}>
            {lastWord}
            <span style={rightQuoteStyle}>”</span>
          </span>
        </Text>
        <div>
          <Text variant="BodyBaskerville" style={nameStyle}>
            {name}{' '}
          </Text>
          <Text variant="ExtraSmallFranklin" style={companyStyle}>
            {company}
          </Text>
        </div>
      </div>
